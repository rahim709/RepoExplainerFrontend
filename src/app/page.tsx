...
import { ArrowRight, GitBranch, MessageSquareCode, Zap, Terminal, Loader2, Sparkles, Search } from "lucide-react";
  ...
  if (checkingAuth) {
    return (
      <div className="min-h-screen bg-[#0d1117] flex items-center justify-center flex-col gap-4">
        ...
        <p className="text-gray-500 text-sm animate-pulse">Initializing RepoExplainer...</p>
      </div>
    );
  }
      ...
      <section className="relative min-h-[90vh] flex flex-col justify-center pt-32 pb-20 px-4 md:pt-40 md:pb-32 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-8 relative z-10">
          ...
          <p className="text-gray-400 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200 px-4">
            Stop grepping through thousands of files. Paste a GitHub link and get an <span className="text-white font-medium">AI Architect</span> that maps your entire project context in seconds.
          </p>
          ...
          <div className="w-full max-w-xl mx-auto mt-12 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300 px-2">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 mt-6 text-xs sm:text-sm text-gray-500">
              ...
              <button onClick={() => setRepoUrl("https://github.com/facebook/react")} className="px-3 py-1.5 rounded-full bg-[#161b22] border border-[#30363d] hover:border-blue-500/50 hover:text-blue-300 hover:bg-blue-500/10 transition-all">
                 ⚛️ facebook/react
              </button>
              <button onClick={() => setRepoUrl("https://github.com/vercel/next.js")} className="px-3 py-1.5 rounded-full bg-[#161b22] border border-[#30363d] hover:border-white/50 hover:text-white hover:bg-white/5 transition-all">
                 ▲ vercel/next.js
              </button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20 space-y-4">
            ...
            <p className="text-gray-400 text-lg max-w-2xl mx-auto">
              We parse the Abstract Syntax Tree (AST) to ensure our AI understands the logic, not just the text.
            </p>
          </div>
      ...
      <section className="py-32 px-4 relative overflow-hidden border-t border-[#30363d]/50">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
           <div className="text-center mb-16">
             ...
             <h2 className="text-3xl md:text-4xl font-bold">From URL to Insight in Seconds</h2>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
             ...
             <Step num="01" title="Paste URL" desc="Simply drop any public GitHub repository link into the search bar." color="text-blue-400" border="group-hover:border-blue-500/50" />
             <Step num="02" title="AI Analysis" desc="Our engine maps the file structure and indexes code logic." color="text-green-400" border="group-hover:border-green-500/50" />
             <Step num="03" title="Start Chatting" desc="Ask complex questions and get answers grounded in truth." color="text-purple-400" border="group-hover:border-purple-500/50" />
           </div>
        </div>
      </section>
...
function FeatureCard({ icon, title, desc, delay, gradient }: { icon: React.ReactNode, title: string, desc: string, delay: string, gradient: string }) {
  return (
    <div 
    ...
    >
      <div className={`absolute top-0 right-0 w-[300px] h-[300px] bg-gradient-to-br ${gradient} blur-[80px] opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`} />
...
function Step({ num, title, desc, color, border }: { num: string, title: string, desc: string, color: string, border: string }) {
  return (
    <div className="text-center relative group">
      <div className={`w-20 h-20 bg-[#0d1117] border border-[#30363d] ${border} rounded-2xl flex items-center justify-center mx-auto mb-8 text-2xl font-bold text-gray-500 group-hover:text-white group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-2xl relative z-10`}>
        <span className={color}>{num}</span>
      </div>
      <h3 className={`text-xl font-bold mb-3 ${color} brightness-90 group-hover:brightness-110 transition-all`}>{title}</h3>
      <p className="text-gray-400 text-sm leading-relaxed px-4 group-hover:text-gray-300 transition-colors">{desc}</p>
    </div>
  );
}