...
import { Send, Paperclip, Bot, User, Terminal, ArrowRight, Copy, Check } from "lucide-react";
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="p-1.5 text-gray-400 hover:text-white hover:bg-[#30363d] rounded-md transition-all opacity-0 group-hover:opacity-100 focus:opacity-100"
      title="Copy message"
    >
...
export const MessageThreadFull = React.forwardRef<HTMLDivElement, MessageThreadProps>(
  ({ className, messages, isLoading, onChatStart, onSendMessage, ...props }, ref) => {
    ...
    if (messages.length === 0 && !isLoading) {
      return (
        <div ref={ref} className={cn("flex flex-col items-center justify-center h-full w-full bg-[#0d1117] p-4 sm:p-8 animate-in fade-in zoom-in duration-300", className)} {...props}>
            ...
            <div className="text-center space-y-6 mb-8 md:mb-10 max-w-2xl w-full pt-12 md:pt-0">
               ...
               <p className="text-[#8b949e] text-base md:text-lg leading-relaxed px-4">
                 Paste a GitHub repository URL below to inspect its architecture, ask questions, and understand the code.
               </p>
            </div>
            ...
            <div className="mt-8 md:mt-10 flex flex-wrap justify-center gap-2 md:gap-3 text-xs md:text-sm text-[#8b949e] px-4">
               <span className="py-1 w-full text-center sm:w-auto">Try popular repos:</span>
               <button onClick={() => onChatStart("https://github.com/facebook/react")} className="px-3 py-1 rounded-full border border-[#30363d] hover:border-blue-500/50 hover:text-blue-400 hover:bg-[#1f6feb]/10 transition-colors">
                  facebook/react
               </button>
               <button onClick={() => onChatStart("https://github.com/vercel/next.js")} className="px-3 py-1 rounded-full border border-[#30363d] hover:border-blue-500/50 hover:text-blue-400 hover:bg-[#1f6feb]/10 transition-colors">
                  vercel/next.js
               </button>
            </div>
        </div>
      ...
      <div ref={ref} className={cn("flex flex-col h-full w-full bg-[#0d1117]", className)} {...props}>
        ...
        <ScrollableMessageContainer className="flex-1 p-4 pt-16 md:p-6 md:pt-6 space-y-6 md:space-y-8 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-700">
            {messages.map((msg, idx) => (
              <div key={idx} className={cn("flex w-full gap-3 md:gap-4 animate-in fade-in slide-in-from-bottom-2 group", msg.role === 'user' ? "justify-end" : "justify-start")}>
                 ...
                 <div className={cn(
                   ...
                   {/* Copy Button */}
                   <div className={cn("absolute top-2 right-2 z-10", msg.role === 'user' ? "text-white/70" : "text-gray-400")}>
                   ...
                   <div className="prose prose-invert max-w-none prose-p:leading-relaxed prose-pre:bg-[#0d1117] prose-pre:border prose-pre:border-[#30363d] prose-pre:rounded-lg prose-pre:text-xs md:prose-pre:text-sm">
                     <ReactMarkdown
                        ...
                        components={{
                          h1: ({node, ...props}) => <h1 className="text-lg md:text-xl font-bold mb-3 md:mb-4 mt-4 md:mt-6 border-b border-[#30363d] pb-2 text-white" {...props} />,
                          h2: ({node, ...props}) => <h2 className="text-base md:text-lg font-bold mb-2 md:mb-3 mt-4 md:mt-5 text-white" {...props} />,
                          h3: ({node, ...props}) => <h3 className="text-sm md:text-base font-semibold mb-2 mt-3 md:mt-4 text-blue-200" {...props} />,
                          a: ({node, ...props}) => <a className="text-[#58a6ff] hover:underline break-all" target="_blank" rel="noopener noreferrer" {...props} />,
                          ...
                          code: ({node, className, children, ...props}) => {
                            ...
                            return isInline ? (
                              <code className="bg-[#6e7681]/20 text-[#c9d1d9] px-1.5 py-0.5 rounded-md text-[85%] font-mono" {...props}>
                                {children}
                              </code>
                            ) : (
                              <div className="relative group mt-4 mb-4">
                                <code className="block bg-[#0d1117] p-3 md:p-4 rounded-lg border border-[#30363d] overflow-x-auto text-xs md:text-sm font-mono leading-normal" {...props}>
                                  {children}
                                </code>
                              </div>
                            )
                          },
                          table: ({node, ...props}) => <div className="overflow-x-auto my-4"><table className="min-w-full border border-[#30363d] rounded-lg" {...props} /></div>,
                          th: ({node, ...props}) => <th className="bg-[#161b22] px-4 py-2 border-b border-[#30363d] text-left font-semibold text-white whitespace-nowrap" {...props} />,
                          td: ({node, ...props}) => <td className="px-4 py-2 border-b border-[#30363d]/50" {...props} />,
                        }}
                     >
                        {msg.content}
                     </ReactMarkdown>
        ...
        <div className="p-3 md:p-4 bg-[#0d1117] border-t border-[#30363d]">
           <div className="max-w-4xl mx-auto relative rounded-xl border border-[#30363d] bg-[#161b22] shadow-lg focus-within:ring-2 focus-within:ring-[#1f6feb] focus-within:border-transparent transition-all">
            ...
            <div className="absolute right-2 bottom-1.5 md:bottom-2 flex items-center gap-1">
              <button className="hidden md:block p-2 text-[#8b949e] hover:text-[#c9d1d9] hover:bg-[#30363d] rounded-lg transition-colors" title="Attach file">
                <Paperclip size={18} />
              </button>
              <button 
